...
Filtering:
  --report-atoms C N O: only report clash/bond errors involving at least one atom of the given element symbols."""
    ...
    def _report_atom(self, atom: 'ob.OBAtom') -> bool:
        """--report-atoms 过滤: 原子是否属于指定元素"""
        if self.report_atom_nums is None:
            return True
        return atom.GetAtomicNum() in self.report_atom_nums
    
    def _check_atom_clashes(self, mol: 'ob.OBMol') -> List[dict]:
        ...
        # 链过滤: 生成保留掩码
        keep = np.array([self._keep_atom(a) for a in atoms], dtype=bool)
        ...
        candidate_pairs = kdtree.query_pairs(2 * max_r * self.args.vdw_scale + 1e-3)
        
        clashes = []
        for i, j in candidate_pairs:
            a1, a2 = atoms[i], atoms[j]
            # --report-atoms: 至少一个原子属于指定元素才报告
            if not (self._report_atom(a1) or self._report_atom(a2)):
                continue
            if self.args.ignore_h and (a1.GetAtomicNum() == 1 or a2.GetAtomicNum() == 1):
                continue
            if self.args.no_hh and a1.GetAtomicNum() == 1 and a2.GetAtomicNum() == 1:
                continue
            ...
            if dist < limit:
                ...
                if self.args.hbond_exempt > 0:
                    ...
                    if h_atom is not None and h_atom.IsPolarHydrogen() \
                        ...
                        if dist > self.args.hbond_exempt:
                            continue
                overlap = limit - dist
                ...
                clashes.append({
                    ...
                    'atom2': atom_repr(a2),
                    'relation': relation,
                    'severity': 'severe' if overlap > self.args.severe_overlap else 'mild',
    ...
    def _check_bond_lengths(self, mol: 'ob.OBMol') -> List[dict]:
        """基于 openbabel GetEquibLength 的共价键键长检测"""
        errors = []
        for bond in ob.OBMolBondIter(mol):
            a1, a2 = bond.GetBeginAtom(), bond.GetEndAtom()
            if not (self._keep_atom(a1) and self._keep_atom(a2)):
                continue
            # --report-atoms: 至少一个原子属于指定元素才报告
            if not (self._report_atom(a1) or self._report_atom(a2)):
                continue
            ...
            tol = max(self.args.bond_tol * eq_length, self.args.bond_tol_abs)
            if diff > tol:
                errors.append({
                    'atom1': atom_repr(a1),
                    'atom2': atom_repr(a2),
    ...
    def _keep_atom(self, atom: 'ob.OBAtom') -> bool:
        r = atom.GetResidue()
        chain = r.GetChain() if r is not None else ''
        if self.args.skip_chains and chain in self.args.skip_chains:
            return False
        if self.args.only_chains and chain not in self.args.only_chains:
            return False
        return True
    
    def _check_one_file(self, pdb_path: Path) -> dict:
        ...
        n_kept = sum(1 for a in ob.OBMolAtomIter(mol) if self._keep_atom(a))
        n_bonds = sum(1 for b in ob.OBMolBondIter(mol)
                      if self._keep_atom(b.GetBeginAtom()) and self._keep_atom(b.GetEndAtom()))
        result = {'file': str(pdb_path), 'atoms': n_kept, 'bonds': n_bonds}
        if not self.args.skip_clash:
            result['clashes'] = self._check_atom_clashes(mol)
            result['n_clashes'] = len(result['clashes'])
        if not self.args.skip_bonds:
            result['bond_errors'] = self._check_bond_lengths(mol)
    ...
    def main_process(self):
        ...
        for pdb_path in tqdm(pdb_paths, total=len(pdb_paths)):
            results.append(self._check_one_file(pdb_path))

...
from lazydock.gmx.abfe.tools import PathLike


def gen_restraint(topology: PathLike,
                  trajectory: PathLike,
                  ligand_selection: str = 'moltype LIG and not name H*',
                  host_selection: str = 'protein and name CA and not moltype LIG',
    ...
    ligand_selection : str, optional
        MDAnalysis selection to define the ligand, by default
        'moltype LIG and not name H*'.  ``moltype`` matches the GROMACS
        moleculetype (which is set to LIG by prepare-abfe for both small
        molecules - whose resname is also LIG - and peptide ligands, whose
        residues keep their protein names GLU/PRO/... by design).
    host_selection : str, optional
        MDAnalysis selection to define the host (receptor), by default
        'protein and name CA and not moltype LIG'.  The ``not moltype LIG``
        clause excludes a peptide ligand, which would otherwise be counted
        as protein (its residues are amino acids).
    temperature : float
